...
function AddNewTask() {
  ...
  deleteButtonInsideTask.innerHTML =
    '<i class="material-icons" style="font-size:1.32vw">delete</i>';
  // plusButtonInsideTask.innerHTML='&plus;';
  plusButtonInsideTask.innerHTML =
    '<i class="fa fa-plus" style="font-size:1.32vw"></i>';
  ...
  this.cardHead = cardHeading;
  this.cardHead.addEventListener("click", () => {
    console.log("Heading clicked");

    document.querySelector(".forOneCardHeadingContaner").style.display =
      "block";
    document.getElementById("mainContainer").style.display = "none";

    card = cardHeading.parentElement.innerHTML;
    
    let card2 = cardHeading.parentElement
    // card2.setAttribute('class', 'newBackgroundForSecondPage')
    // card = card.innerHTML

    console.log("card2 is : ", card2);
    console.log("card2  next sibling below: ");

    let deleteButton = card2.children[2].children[0]
    let addButton = card2.children[2].children[1]
    console.log("DEkete " , deleteButton)
    console.log("adsd ", addButton)
    card2.children[2].children[0].addEventListener("click", ()=>{
      console.log("on delete methd of de")
     console.log( deleteButton .parentElement.parentElement.parentElement)

    })
    document.getElementById("cardThatIsSelectedCeneter").innerHTML = card;
    document.getElementById("HeadingOfCardInsideGrid").innerText =
      valueofCardCreated;
      
    // document.getElementById('cardThatIsSelectedCeneter').insertAdjacentElement("afterbegin",cardHeading.parentElement);
    // document
    //   .getElementById("cardThatIsSelectedCeneter")
    ...
    document
      .getElementById("BackButtonForSingleCard")
    ...
    //   document.querySelector(".material-icons").parentElement.parentElement.parentElement.remove();
    // });


    // this.cardHead.firstChild.addEventListener("click", ()=>{
      
    //       this.cardHead.parentElement.parentElement.parentElement.remove();
    //      }
...
function AddNewTaskForCountry() {
  ...
  document
    .getElementById("closeButtonPopUpScreenForCountry")
    .addEventListener("click", function ClosePopUpScreenForCountry() {
      document.getElementById("popUpForCountry").style.display = "none";
      document.getElementById("mainContainer").style.filter = "blur(0px)";
      // console.log("Close pop for country invoked : ", this.parentElement.parentElement);
    });

  // ouetr: const hrel = this.parentElement.parentElement;
  let hrel = this.parentElement.parentElement;
  console.log("add button for country invoked ", hrel);

  document
    .getElementById("addButtonPopUpScreenForCountry")
    .addEventListener("click", () => {
      ...
      let activityValue = document.getElementById(
        "inputTaskIdForCountry"
      ).value;
